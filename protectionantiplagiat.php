...
foreach ($bannav as $banned) { $comparaison = strstr($useragent, $banned);
    if($comparaison !== false) {
       $write_this = '[Information] Aspirateur : '.$useragent.' Host : '.$hostname.' Adresse ip : ' .getip(); // Le texte que vous voulez avoir dans votre fichier protectionantiplagiat.cnx.
       $tentative++;
    }
}
//----------------------------------------------------
...
if($tentative > 0){
   ...
   fwrite($write_here, "\n" . $write_this);
   fclose($write_here);
   echo utf8_decode( '[Sécurité] Notre site web est protégé, vos information sont enregistrer <br><br>[Information] : '.$useragent.' '.$hostname.' '.getip().''); // Le texte que vous voulez que le voleur recevra dans les fichiers télécharger.
